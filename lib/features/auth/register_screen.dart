  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (AuthService.isPhone(_identifier.text)) {
        setState(() {
          _awaitingOtp = true;
          _notice = 'We sent a 6-digit code by SMS. Enter it below to finish.';
        });
      } else {
        setState(() => _notice =
              ...
              children: [
                ...
                TextFormField(
                  ...
                  validator: (v) {
                    ...
                    if (!looksEmail && !AuthService.isPhone(value)) {
                ...
                if (_notice != null) ...[
                  ...
                  _Banner(message: _notice!, color: AppTheme.signalTeal),
                ],
                if (_error != null) ...[
                  const SizedBox(height: 16),
                  _Banner(message: _error!, color: AppTheme.hibiscus),
                ],
                const SizedBox(height: 24),
                FilledButton(
                  onPressed:
                      _busy ? null : (_awaitingOtp ? _verify : _submit),
                  child: Text(_awaitingOtp ? 'Verify code' : 'Create account'),
                ),
              ],
            ),
          ),
