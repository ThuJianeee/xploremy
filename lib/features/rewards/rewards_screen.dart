...
class _RewardsScreenState extends State<RewardsScreen> {
  ...
  Future<void> _claim(String id, int cost) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(ok
            ? 'Reward claimed.'
            : 'Not enough XP or reward already claimed.')));
    await _load();
  }
                  ...
                  child: Column(
                    ...
                    children: [
                      const Icon(Icons.error_outline,
                          size: 40, color: AppTheme.hibiscus),
                      const SizedBox(height: 12),
                      const Text('Unable to load rewards.',
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
            ...
            child: Padding(
              ...
              child: Column(
                  ...
                  children: [
                    Row(children: [
                      Expanded(
                          child: Text('Level ${snapshot.level}',
                    ...
                    const SizedBox(height: 8),
                    Text(
                        '${snapshot.nextLevelXp - snapshot.xp} XP to next level · ${snapshot.streak} day streak 🔥',
          ...
          for (final mission in missions)
            Padding(
                ...
                child: Card(
                  ...
                  subtitle: Text(
                      '${mission.progress.clamp(0, mission.target)}/${mission.target} · +${mission.xp} XP'),
                  trailing: SizedBox(
                      width: 70,
          ...
          const SizedBox(height: 10),
          _RewardTile(
              title: 'Transit Explorer Theme Badge',
              cost: 250,
              claimed: snapshot.claimedRewards.contains('theme_badge'),
              onClaim: () => _claim('theme_badge', 250)),
          _RewardTile(
              title: 'Eco Commuter Profile Flair',
              cost: 500,
              claimed: snapshot.claimedRewards.contains('eco_flair'),
              onClaim: () => _claim('eco_flair', 500)),
          const Padding(
              padding: EdgeInsets.only(top: 10),
              child: Text(
                  'Rewards are in-app achievements only and have no cash value.',
